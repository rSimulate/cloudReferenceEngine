...
var mongoUri = process.env.MONGOLAB_URI || 'mongodb://localhost/cloudReferenceEngine';
...
mongo.connect(mongoUri, {}, function(error, db) {
	...
	app.post('/metasim/:version/simulations', function(request, response) {
		...
		response.header('Location', url.format({
			...
			port: port,
			pathname: request.originalUrl + '/' + simulationId + '/data.png'}));
	...
	app.get('/metasim/:version/simulations/:id/data.png', function(request, response) {
		response.sendfile('data.png');
	});
});
